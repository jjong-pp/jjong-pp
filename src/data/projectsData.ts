...
| **NICE평가정보** | 본인인증 · 통관검증 중계 | 본인인증사 담당자 |
...
| 통관 파트너사 | GGATE API V2 (통관 파트너사 신고 API) | **v1.2.0** (26.06.10) | HWB 단건/벌크/조회 |
...
sequenceDiagram
    ...
    participant Z as 통관 파트너사 (Giant)
    ...
    Note over G, D: 매 영업일 11:00 현지 창고 수집 / 11:30, 12:00 Geek 폴링
...
| API 호출 | ① 현지 창고 WMS 수집 후 HBL(송장번호) 채번 및 패킹 시작 → ② **공식몰(Geek) 스케줄러가 11:30 / 12:00 (하루 2회)** 현지 창고 API를 호출하여 채번된 HBL과 **상품의 실측 무게(Weight)** 수신 → ③ 고도몰 DB에 HBL 및 무게 적재 |
...
**오류 발생 시 자동화 흐름:**
1. TRA003에서 에러 건 발견
2. 고도몰 어드민에 **\`[출고보류(Hold)]\`** 플래그 자동 세팅 → 현지 창고 송장 출력 원천 차단
...
| 통관 결과 폴링 | 물리적인 상품 준비(출고 예정/완료) 후 **3일(T+3일) 시점**에 통관 파트너사 API를 호출하여 최종 통관/배송 상태 결과값 수신 |
...
- [ ] **레이스 컨디션 및 트랜잭션**: 현지 창고 배치와 고도몰 상태 전환 타이밍 충돌 시 DB 트랜잭션 락/데드락 발생 없이 g1 수집 방어 여부
...
- [ ] **통관 파트너사 에러 응답 파싱**: 통관 파트너사 HTTP 500/400 에러 시 Response Body 내의 상세 사유가 고도몰 117/118번 컬럼의 VARCHAR 길이 제한을 초과하지 않고 안전하게 바인딩(TRUNCATE) 되는지 점검
...
| 5 | 연동 | **현지 창고-고도몰 상태 레이스 컨디션** | 현지 창고 배치와 고도몰 주문 상태 전환의 타이밍 충돌 | 치명적인 이중출고 (대형 물류사고) | 초기 1주간 매일 DB 대사 스크립트 실행. p1→g1 전환 시 DB 락으로 방어 |
...
| 7 | 연동 | **통관 파트너사 500건 벌크 미검증** | GGATE API가 단건→500건 확장되었으나 실제 대량 테스트 미진행 | 응답 지연/타임아웃으로 전건 실패 처리 | 100건 청크 + 타임아웃 30초 방어 + 통관 파트너사 담당자와 사전 부하 테스트 협의 |
...
| 13 | 인증 | **NICE AES/GCM 복호화 키 변경** | NICE 측에서 암호화 키를 사전 통보 없이 갱신할 경우 | SMARTPCC_11 전면 장애 → 결제 전면 차단 | NICE 담당 매니저와 키 변경 통보 SLA 사전 합의 |
...
projectsFullMarkdown['blog'] = `#  GitHub Pages에 완벽하게 맞추기 위한 아키텍처 변경 방안
