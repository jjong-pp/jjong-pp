...
- 수동 송장 업데이트 차단: 고도몰 주문 상세에서 CS팀이 송장번호만 단순 수정/업데이트하는 행위는 시스템 차원에서 원천 차단합니다. (단순 업데이트 시 관세청에는 과거 송장으로 제출된 상태로 남아 데이터가 심각하게 꼬입니다)
...
웨이하우스별로 도착지 우편번호를 기준 거리와 예상 소요시간을 데이터들을 중첩시켜 평균 중위값을 자동으로 측정하여 예상 소요시간 리드타임 표준 정립
	- 거리는 naverapi를 이용하여 실시간으로 4가지 (실시간빠른길,편한길, 최단거리, 무료우선 중)  2번째로 긴 거리를 선정
### 프로세스 설계
\`\`\`mermaid
graph LR
    subgraph 로그인_인증
