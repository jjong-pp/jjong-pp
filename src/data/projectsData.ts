...
sequenceDiagram
    ...
    alt [에러⑤] 현지 창고 API 타임아웃/응답 없음
        ...
        Note right of G: ▶해결: 15분 후 자동 재시도. 3회 실패 시 사내 메신저 알림 + 수동 확인
...
| \`domesticCarrierName\` | 국내 택배사 | 국내 배송사 |
...
| 영문정보(7.3) | 영문주소: 행안부 API. 영문성명: 로마자 추천+수동입력 | 기계번역 API 기각. 2일 공수 |
...
- [ ] 방화벽 ANY 오픈 확인: 관세청 통합테스트 전 운영 대역 방화벽 정상 오픈 여부 확인
...
| 3 | 인프라 | 방화벽 ANY 오픈 미확정 | 관세청이 통합테스트 전 방화벽 ANY 오픈 시점을 별도 통보 예정이나 아직 미통보 | 방화벽 미오픈 시 연동 테스트 자체 진행 불가 | 관세청 담당 창구로 주간 단위 일정 재확인 메일 발송 |
