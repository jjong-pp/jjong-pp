...
- **프로젝트 결과물은 실제 자료가 확인되면 붙인다.** 프로젝트별 GitHub 저장소, 시연 영상,
  개발 결과 화면을 상세 페이지의 `결과물 및 참고자료`에 연결한다. 없는 자료는 임의로 만들지 않는다.
  전역 프로필 링크와 프로젝트 결과물 링크를 섞지 않는다.
...
```
src/data/profile.ts        소개·이력·학력·프로젝트 메타·결과물 첨부 (콘텐츠 단일 소스)
...
- **2026-08-09** 이력서 기준으로 재정비. 대외비 45건 익명화, GitHub 링크 전량 삭제,
  ...
  초기 번들 892KB → 261KB.
- **2026-08-17** 삭제됐던 프로젝트 결과물 연결을 복구. B2B·SCM 결과 화면과 TMS GitHub·시연 영상을
  프로젝트 메타에서 관리하고, 첨부가 없는 프로젝트에는 섹션을 표시하지 않도록 정리.
